...
public class ExcelDataLoader {
	...
	public static <T> List<T> selectQuery(Class<T> type, String query, String FilePath) throws SQLException {
		List<T> list = new ArrayList<T>();
		try {
			Connection data = _excelReaderInstance.getConnection(FilePath);
			Recordset rs = data.executeQuery(query);
			while (rs.next()) {
	...
	private static void loadResultSetIntoObject(Recordset rs, Object object)
		...
		for (Field field : zclass.getDeclaredFields()) {
			...
			Object value = rs.getField(ExcelColumn.ExcelColumn());
